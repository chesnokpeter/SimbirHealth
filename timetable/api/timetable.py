...
async def create_appointments(
    id: int, data: AppointmentsCreate, token=Security(get_token), tt=Depends(get_timetrepo), at=Depends(get_apporepo)
) -> AppoinimentModel:
    u = await introspection(token)
    uow = uowdep(tt, at)()
